...
/**
 * Fetches {@code hive -> starrocks} resource mappings published by RMM. A mapping location is the
 * StarRocks name of the Hive entity ({@code catalog.database[.table]}); it is converted to the
 * path form used by the mapping store.
 */
public class StarRocksHiveMappingFetcher extends HiveMappingFetcher {
    public StarRocksHiveMappingFetcher(RangerAdminClient adminClient,
    ...
    protected ResourceMapping transformMapping(ResourceMapping mapping) {
        return new ResourceMapping(
            mapping.getName(),
            StarRocksResourcePath.fromLocation(mapping.getLocation())
        );
    }
}
