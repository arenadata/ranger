...
/**
 * Re-applies the embedded Trino service-definition so that already-installed
 * clusters pick up the "execute" and "grant" access types that ADH-8121 added
 * to the schemafunction resource (accessTypeRestrictions). The previous Trino
 * service-def patch (J10062) does not re-run on clusters where it is already
 * marked applied in x_db_version_h, hence this new versioned patch.
 */
@Component
